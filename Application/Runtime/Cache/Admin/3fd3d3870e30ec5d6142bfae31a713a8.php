                ...
                <div class="panel-body">
                    <form role="form" action="<?php echo U('edit', ['id' => $info['id']]);?>" method="post">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="exampleInputEmail1" name="title" value="<?php echo ($info["title"]); ?>" placeholder="请输入名字">
                        </div>
                        <div class="form-group">
                            <label for="exampleInputEmail1">文章所属分类</label>
                            <select class="form-control" name="category_id">
                                <?php if(is_array($categories)): $i = 0; $__LIST__ = $categories;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$c): $mod = ($i % 2 );++$i;?><option value="<?php echo ($c["id"]); ?>" <?php if($info['category_id'] == $c['id']): ?>selected<?php endif; ?> ><?php echo ($c["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="exampleInputEmail1">文章简介</label>
                            <textarea name="intro" class="form-control"><?php echo ($info["intro"]); ?></textarea>
                        </div>
                        <div class="form-group">
                            <label>文章点击量</label>
                            <input type="text" class="form-control" id="" name="hits" placeholder="" value="<?php echo ($info["hits"]); ?>">
                        </div>
                        <div class="form-group">
                            <label for="exampleInputEmail1">文章详细内容</label>
                            <textarea id="container" name="content"><?php echo ($info["content"]); ?></textarea>
                        </div>
                        <div class="checkbox">
                            <label>
