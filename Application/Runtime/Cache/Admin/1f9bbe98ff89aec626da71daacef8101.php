                ...
                <div class="panel-body">
                    <form role="form" action="<?php echo U('edit', ['id' => $info['id']]);?>" method="post">
                        <div class="form-group">
                            <label>权限名称</label>
                            <input type="text" class="form-control" name="name" placeholder="请输入名字" value="<?php echo ($info["name"]); ?>">
                        </div>
                        <div class="form-group">
                            <label>权限URL地址</label>
                            <input type="text" class="form-control" name="url" placeholder="" value="<?php echo ($info["url"]); ?>">
                        </div>
                        <div class="form-group">
                            <label>所属上级</label>
                            <select name="parent_id" class="form-control">
                                <option value="0">最顶级</option>
                                <?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><option value="<?php echo ($list["id"]); ?>" <?php if($info['parent_id'] == $list['id']): ?>selected<?php endif; ?>>
                                        <?php $__FOR_START_14799__=1;$__FOR_END_14799__=$list['level'];for($i=$__FOR_START_14799__;$i < $__FOR_END_14799__;$i+=1){ ?>&nbsp; &nbsp; &nbsp;<?php } ?>
