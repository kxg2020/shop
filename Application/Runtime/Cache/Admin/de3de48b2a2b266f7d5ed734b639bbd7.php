        ...
        <div class="col-lg-12">
            <section class="panel">
                ...
                <div class="panel-body">
                    <form role="form" action="<?php echo U('permission', ['id' => $info['id']]);?>" method="post">
                        <div class="form-group">
                            <label>角色名称</label>
                            <span class="text-danger"><b><?php echo ($info["role_name"]); ?></b></span>
                        </div>
                        <div class="form-group">
                            <label>权限列表</label><br/>
                            <?php if(is_array($pers)): $i = 0; $__LIST__ = $pers;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$p): $mod = ($i % 2 );++$i; $__FOR_START_3761__=1;$__FOR_END_3761__=$p['level'];for($i=$__FOR_START_3761__;$i < $__FOR_END_3761__;$i+=1){ ?>&nbsp;&nbsp;<?php } ?>
                                <input type="checkbox" <?php if(in_array($p['id'], $permissions)): ?>checked<?php endif; ?> name="p[]" value="<?php echo ($p["id"]); ?>" /><?php echo ($p["name"]); ?><br/><?php endforeach; endif; else: echo "" ;endif; ?>
                        </div>
